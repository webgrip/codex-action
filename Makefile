...
start:
	...
	@$(COMPOSE) run --rm codex-action.lint || { echo "Lint failed, running make fix..."; $(MAKE) fix || { echo "make fix failed"; exit 1; }; echo "Rebuilding images and starting stack again..."; $(COMPOSE) up --build -d || { echo "Compose up failed after fixes"; exit 1; }; }
	@echo "Compose is up."
...
enter:
	@$(COMPOSE) exec codex-action.test /bin/sh || $(COMPOSE) exec codex-action.lint /bin/sh
...
test:
	@$(COMPOSE) run --rm codex-action.test

lint:
	@$(COMPOSE) run --rm codex-action.lint
